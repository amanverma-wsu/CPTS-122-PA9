...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        obj[2].draw(window);
        obj[3].draw(window);
	std::vector<Shape*> diceShapes;

        // Draw rectangles for the dice values
        // Clear the previous dice shapes
diceShapes.clear();

// Draw shapes for the dice values using polymorphism
for (size_t i = 0; i < gameState.diceStore[playerID].size(); ++i) {
    Shape* shape;
    int diceValue = gameState.diceStore[playerID][i];
    float posX = 776 + i * 54;
    float posY = 553;

    // Alternate between circle and square shapes
    if (i % 2 == 0) {
        shape = new CircleShape(25.f, sf::Color::Yellow, posX, posY);
    } else {
        shape = new SquareShape(50.f, sf::Color::Cyan, posX, posY);
    }

    diceShapes.push_back(shape);
}

// Draw the shapes polymorphically
for (Shape* shape : diceShapes) {
    shape->draw(window);
}

// Draw the dice values on top of the shapes
for (size_t i = 0; i < gameState.diceStore[playerID].size(); ++i) {
    int diceValue = gameState.diceStore[playerID][i];
    float posX = 776 + i * 54;
    float posY = 553;
    ...
    sf::Text text;
    text.setFont(font);
    text.setString(std::to_string(diceValue));
    text.setCharacterSize(24);
    text.setFillColor(sf::Color::Red);
    text.setPosition(posX + 15.f, posY + 15.f);
    window.draw(text);
}

// After drawing, delete the shapes
for (Shape* shape : diceShapes) {
    delete shape;
}
diceShapes.clear();
